...
const updateProfileSchema = Joi.object({
    ...
    year: Joi.string().valid('Freshman', 'Sophomore', 'Junior', 'Senior', 'Graduate')
});
...
router.get('/:id/statistics', (req, res) => {
    try {
        ...
        const stats = {
            ...
            pointsPerUpload: user.totalUploads > 0 ? Math.round(user.points / user.totalUploads) : 0,
            downloadToUploadRatio: user.totalUploads > 0 ? Math.round(user.totalDownloads / user.totalUploads * 100) / 100 : 0
        };
